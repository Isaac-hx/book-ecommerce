...
func GetAllOrderItems(c *gin.Context) {
	...
	if err := db.Preload("Book.Author").Preload("Book.Publisher").Preload("Profile").Find(&orderItems).Error; err != nil {
...
func GetOrderItemsById(c *gin.Context) {
	...
	if err := db.Preload("Book.Author").Preload("Book.Publisher").Preload("Profile").First(&orderItem, id).Error; err != nil {
